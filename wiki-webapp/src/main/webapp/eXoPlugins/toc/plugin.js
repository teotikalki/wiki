...
//							content += "<a href='#" + node.$.id + "'>" + node.$.innerHTML + "</a><br/>";
				...
				*/
				...
				var open = editor.widgets.toc && (editor.widgets.toc.data.numbered=="Yes")? "<ol>" : "<ul>"; 
				var close = editor.widgets.toc && (editor.widgets.toc.data.numbered=="Yes")? "</ol>" : "</ul>";
				var content = open;
				var print = function(elem, indent) {
					for (var i = 0; i < elem.length; i++) {
						content +="<li><a href='#" + elem[i].$.id + "'>" + elem[i].$.innerHTML + "</a>";
						console.log(indent + elem[i].getName());
						if (elem[i].child) {
							content += open;
							print(elem[i].child, indent + " ");
							content += close;
						}
						content+="</li>";
					}
